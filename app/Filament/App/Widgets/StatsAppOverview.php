...
class StatsAppOverview extends BaseWidget
{
    ...
    {
        return [
            Stat::make('Users', Company::find(Filament::getTenant())->first()->members->count())
                ->description('All users from the database')
                ->descriptionIcon('heroicon-m-arrow-trending-up')
                ->chart([7, 2, 10, 3, 15, 4, 17])
                ->color('success'),
            Stat::make('Stores', Store::query()->whereBelongsTo(Filament::getTenant())->count())
                ->description('Empresas en database')
