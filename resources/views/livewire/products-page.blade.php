	...
	<div class="container">
		<div class="row">
			<div class="col-8 mx-auto text-center">
				<h2 class="mb-3 text-capitalize">Posts de Productos</h2>
				<ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
					<li class="list-inline-item"><a wire:navigate href="/">Inicio</a>
					</li>
					<li class="list-inline-item">/ &nbsp; <a wire:navigate href="{{route('items')}}">Lista de Productos</a>
					</li>
				</ul>
			</div>
		</div>
			...
			<div class="col-lg-9">
				<div class="me-lg-4">
					<div class="row gy-5">
						...
						<div class="col-md-6" data-aos="fade">
							<article class="blog-post">
								<div class="post-slider slider-sm rounded">
									<img loading="lazy" decoding="async" src="{{$item->getThumbnail()}}" alt="Post Thumbnail">
								</div>
								<div class="pt-4">
                                    @foreach ($item->categories as $category)
                                        <a href="{{route('items').'?categorySlug='.$category->slug}}" class="pb-4 text-sm font-bold text-blue-700 uppercase">
                                            {{$category->name}}
                                        </a>
                                    @endforeach
									<p class="mb-3">{{ \Carbon\Carbon::parse($item->published_at)->format('F jS Y') }}</p>
									<h2 class="h4"><a class="text-black" href="{{route('item',$item->id)}}">{{$item->title}}</a></h2>
									<p>{!! \Illuminate\Support\Str::limit($item->content,50,'...') !!}</p>
                                    <a href="{{route('item',$item->id)}}" class="text-primary fw-bold" aria-label="Read the full article by clicking here">Ver Más</a>
								</div>
							</article>
						</div>
						...
						<div class="col-12">
							<nav class="mt-4 pagination justify-content-center">
                                    <!-- pagination -->
                                {{$posts->links()}}
							</nav>
						</div>
			...
			<div class="col-lg-3">
				...
				<div class="widget widget-categories">
					...
					<ul class="list-unstyled widget-list">
						...
						<li><a wire:navigate href="{{route('items').'?categorySlug='.$category->slug}}">{{$category->name}}    <small class="ml-auto">({{$category->total}})</small></a>
						</li>
                        @endforeach
                        @endif
				...
				<!-- latest post -->
				<div class="widget">
					<h5 class="widget-title"><span>Ultimas publicaciones</span></h5>
					<!-- post-item -->
              		<?php $posts=App\Models\Post::orderBy('created_at','DESC')->get()->take(4) ?>
                        @foreach ($posts as $item)
					<ul class="list-unstyled widget-list">
						<li class="d-flex widget-post align-items-center">
							<a class="text-black" href="/blog/elements/">
								<div class="widget-post-image flex-shrink-0 me-3">
									<img class="rounded" loading="lazy" decoding="async" src="{{$item->getThumbnail()}}" alt="Post Thumbnail">
								</div>
							</a>
							<div class="flex-grow-1">
								<h5 class="h6 mb-0">
                                    <a class="text-black" href="blog-details.html">{{$item->title}}</a></h5>
								<small>{{ \Carbon\Carbon::parse($item->published_at)->format('F jS Y') }}</small>
							</div>
						</li>
					</ul>
